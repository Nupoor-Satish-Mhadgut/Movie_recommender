import pandas as pd
import ast
from sklearn.feature_extraction.text import CountVectorizer
from sklearn.metrics.pairwise import cosine_similarity
import requests
from dotenv import load_dotenv
import os
import gc
from functools import lru_cache

load_dotenv()

# --- Memory Optimized Data Loading ---
def load_and_filter_data():
    # Load only the first 3000 movies to reduce memory
    movies = pd.read_csv(
        os.path.join('movies', 'movies_metadata.csv'),
        low_memory=False,
        usecols=['id', 'title', 'overview', 'genres']
    ).head(3000)
    
    # Load and merge other datasets
    credits = pd.read_csv(
        "https://huggingface.co/datasets/nupoorm/credits-dataset/resolve/main/credits.csv",
        usecols=['id', 'cast', 'crew']
    )
    keywords = pd.read_csv(
        os.path.join('movies', 'keywords.csv'),
        usecols=['id', 'keywords']
    )
    links = pd.read_csv(
        os.path.join('movies', 'links.csv'),
        usecols=['tmdbId']
    )
    
    # Clean and merge
    for df in [credits, keywords, links]:
        movies = movies.merge(df, how='left', left_on='id', right_on='id' if 'id' in df.columns else 'tmdbId')
    
    return movies

movies = load_and_filter_data()

# --- Memory Efficient Processing ---
def process_movies(movies):
    # Convert to efficient dtypes
    movies = movies.astype({
        'id': 'int32',
        'title': 'category',
        'overview': 'string'
    })
    
    # Parsing functions with memory cleanup
    def safe_parse(x):
        try:
            return [i['name'] for i in ast.literal_eval(x)]
        except:
            return []

    movies['genres'] = movies['genres'].apply(safe_parse)
    movies['keywords'] = movies['keywords'].apply(safe_parse)
    movies['cast'] = movies['cast'].apply(lambda x: safe_parse(x)[:3])  # Top 3 cast only
    movies['crew'] = movies['crew'].apply(
        lambda x: next((i['name'] for i in ast.literal_eval(x) if i['job'] == 'Director'), None)
    
    # Create combined text feature
    movies['soup'] = (
        movies['overview'].fillna('') + ' ' +
        movies['genres'].apply(' '.join) + ' ' +
        movies['keywords'].apply(' '.join) + ' ' +
        movies['cast'].apply(' '.join) + ' ' +
        movies['crew'].fillna('')
    )
    
    return movies

movies = process_movies(movies)

# --- Optimized Similarity Calculation ---
def compute_similarity():
    count = CountVectorizer(stop_words='english', max_features=5000)  # Limit vocabulary
    count_matrix = count.fit_transform(movies['soup'])
    
    # Compute similarity in chunks if needed
    cosine_sim = cosine_similarity(count_matrix, count_matrix)
    
    # Clean up
    del count_matrix
    gc.collect()
    
    return cosine_sim

cosine_sim = compute_similarity()

# --- Cache API Responses ---
TMDB_API_KEY = os.getenv("TMDB_API_KEY")

@lru_cache(maxsize=1024)
def fetch_movie_details(tmdb_id):
    if pd.isna(tmdb_id) or not tmdb_id:
        return "", "", ""
    
    try:
        response = requests.get(
            f"https://api.themoviedb.org/3/movie/{int(tmdb_id)}",
            params={'api_key': TMDB_API_KEY, 'language': 'en-US'},
            timeout=5
        )
        data = response.json()
        return (
            f"https://image.tmdb.org/t/p/w500{data.get('poster_path', '')}",
            ', '.join(g['name'] for g in data.get('genres', [])),
            data.get('release_date', '')[:4]
        )
    except:
        return "", "", ""

# --- Memory Efficient Recommendation Function ---
def recommend(title, cosine_sim=cosine_sim):
    indices = pd.Series(movies.index, index=movies['title']).drop_duplicates()
    
    if title not in indices:
        return [], [], [], [], []
    
    idx = indices[title]
    sim_scores = sorted(enumerate(cosine_sim[idx]), key=lambda x: x[1], reverse=True)[1:6]
    
    # Process one movie at a time to reduce memory spikes
    results = []
    for i, score in sim_scores:
        movie = movies.iloc[i]
        poster, genre, year = fetch_movie_details(movie.get('tmdbId'))
        trailer = fetch_trailer(movie.get('tmdbId')) if pd.notna(movie.get('tmdbId')) else ""
        results.append((movie['title'], poster, genre, year, trailer))
    
    # Transpose results into separate lists
    return tuple(zip(*results)) if results else ([], [], [], [], [])